...
fun main(args: Array<String>) {
    ...
    runBlocking {
        val ffmpegimgtovideoutilES = FFmpegImgToVideoA1ES()
...
//        }
        launch(Dispatchers.IO) {
            ...
            ffmpegimgtovideoutilES.awsOutFileDirName = "A1-Audio-ES"
                ...
                fileList.forEachIndexed { index, audioFile ->
                    ...
                    launch(Dispatchers.IO) {
                        ...
                        if (isEnglish) {
                            ...
                            val localAudioFile = File(audioFile.path.replace(".mp3", "-es.mp3"))
    ...
    private fun compressAudio(cmdPath: String, subList: List<SortedExcelData2>) {
        runBlocking {
            for (listFile in File(inputAudioDirPath).listFiles().filter { file ->
                ...
                launch(Dispatchers.IO) {
                    val slowAudioFile =
                        File("$workingDir/aws-audio/$awsOutFileDirName/${(listFile.name.split(".")[0])}-es.mp3")
                    val tempOutputFile =
        ...
        for (sortedExcelDataList in Utils.averageAssignFixLength(subList, 10)) {
            runBlocking {
                for (sortedExcelData in sortedExcelDataList) {
                    val localAudioFile =
                        File("$workingDir/aws-audio/$awsOutFileDirName/" + "yt-en-a1vocab-${sortedExcelData.ID}" + "-es.mp3")
